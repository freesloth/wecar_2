...
class pure_pursuit:
    ...
    def __init__(self):
        ...
        self.servo_pub =rospy.Publisher('commands/servo/positon',Float64,queue_size=1)
        ...
        self .steering_angle_to_servo_gain=-1.1235
        self .steering_angle_to_servo_offset=-0.5304
        rate=rospy.Rate(30)

        while not rospy.is_shutdown():
            if self.is_path==True and(self.is_odom==True or self.is_amct==True):
                
                vechcle_position=self.current_position
                rotated_point=point()
                self.is_look_forward_point=False

                for num,i in enumerate(self.path.poses):
                    path_point=i.pose.position
                    dx=path_point.x -vechcle_position.x
                    dy=path_point.y -vechcle_position.y
                    rotated_point.x=cos(self.vehicle_yaw)*dx +sin(self.vehicle_yaw)*dy
                    rotated_point.y=cos(self.vehicle_yaw)*dx -cos(self.vehicle_yaw)*dy
    ...
    def odom_callback(self,msg):
            self.is_odom=True
            odom_quaternion=(msg.pose.pose.orientation.x,msg.pose.pose.orientation.y,msg.pose.pose.orientation.x,msg.pose.pose.orientation.z,msg.pose.pose.orientation.w)
    ...
    def amcl_callback(self,msg):
            self.is_amcl=True
            amcl_quaternion=(msg.pose.pose.orientation.x,msg.pose.pose.orientation.y,msg.pose.pose.orientation.x,msg.pose.pose.orientation.z,msg.pose.pose.orientation.w)
