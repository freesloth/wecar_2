...
class LASERParser:
    ...
    def callback(self, msg):
        ...
        if back_range_max < 0.9: self.back_detect = True
        else: self.back_detect = False
...
TRRIGER_ONLOAD = 0
TRRIGER_RIGHT_SIDE_RUN = 1
TRRIGER_RIGHT_SIDE_STAY = 2
TRRIGER_REONLOAD_STAY = 3
action_trigger = TRRIGER_ONLOAD
sens_back_cnt = 0
sens_left_cnt = 0
sens_back = False
sens_left = False
...
def mosei_control(sens_back_in, sens_left_in, set_spd, set_steer):
    global action_trigger, sens_back_cnt, sens_left_cnt, sens_back, sens_left
    global trig_side_run, trig_stay_run, trig_reon_run
    ...
    if sens_back and not sens_left:
        if trig_side_run == 0.0 and trig_stay_run == 0.0 and trig_reon_run == 0.0:
            trig_side_run = curTime + 5.0
            action_trigger = TRRIGER_RIGHT_SIDE_RUN
    elif not sens_back and sens_left:
        if trig_side_run == 0.0 and trig_stay_run != 0.0 and trig_reon_run == 0.0:
            trig_stay_run = curTime + 2.0
    elif not sens_back and not sens_left:
        if trig_side_run == 0.0 and trig_stay_run == 0.0 and trig_reon_run == 0.0:
            action_trigger = TRRIGER_ONLOAD
    else:
        if trig_side_run == 0.0 and trig_stay_run != 0.0 and trig_reon_run == 0.0:
    ...
    if trig_side_run <= curTime and trig_side_run != 0.0:
        trig_side_run = 0.0
        action_trigger = TRRIGER_RIGHT_SIDE_STAY
        trig_stay_run = curTime + 2.0
    
    if trig_stay_run <= curTime and trig_stay_run != 0.0:
        trig_stay_run = 0.0
        action_trigger = TRRIGER_REONLOAD_STAY
        trig_reon_run = curTime + 5.0

    if trig_reon_run <= curTime and trig_reon_run != 0.0:
        trig_reon_run = 0.0
        action_trigger = TRRIGER_ONLOAD
    ...
    #         active_enabled = True
    #         next_trigger_delay_outter = get_time + 5.0
    #         action_trigger = TRRIGER_RIGHT_SIDE_RUN
    #     elif next_trigger_delay_outter == 0.0:
    #         action_trigger = TRRIGER_RIGHT_SIDE_STAY
    #     else:
    #         if sens_left:
    #             if not active_enabled or next_trigger_delay_outter == 0.0:
    #                 action_trigger = TRRIGER_RIGHT_SIDE_STAY
    #         else:
    #             action_trigger = TRRIGER_RIGHT_SIDE_RUN
    ...
    # elif not sens_back and not sens_left:
    #     if active_enabled:
    #         if next_trigger_delay_restore == 0.0:
    #             next_trigger_delay_restore = get_time + 5.0
    #         action_trigger = TRRIGER_REONLOAD_STAY
    #     else:
    #         action_trigger = TRRIGER_ONLOAD
    #     print("none")

    # # trigger next changer
    # if next_trigger_delay_outter <= get_time and next_trigger_delay_outter != 0.0:
    #     next_trigger_delay_outter = 0.0
    #     action_trigger = TRRIGER_RIGHT_SIDE_STAY
    #     print("trig_out")

    # if next_trigger_delay_restore <= get_time and next_trigger_delay_restore != 0.0:
    #     next_trigger_delay_restore = 0.0
    #     active_enabled = False
    #     print("trig_res")

    #trigger result run
    if action_trigger == TRRIGER_ONLOAD: #Default Line Drive
        set_spd = set_spd
        set_steer = set_steer
        print("action_trigger ONLOAD", sens_back, sens_left)
    elif action_trigger == TRRIGER_RIGHT_SIDE_RUN: #Right Line Drive
        set_steer = 0.85
        print("action_trigger SIDE RUN", sens_back, sens_left)
    elif action_trigger == TRRIGER_RIGHT_SIDE_STAY: #Right Line Staing
        set_steer = 0.5304
        set_spd = 0
        print("action_trigger SIDE STAY", sens_back, sens_left)
    elif action_trigger == TRRIGER_REONLOAD_STAY: #Right Line Reload start
        set_steer = 0.15
        print("action_trigger REONLOAD", sens_back, sens_left)
    
    return set_spd, set_steer
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        set_speed, dst_steer_level = mosei_control(is_backside, is_leftside, set_speed, dst_steer_level)
