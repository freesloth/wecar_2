...
class LASERParser:
    ...
    def callback(self, msg):
        ...
        '''
        max_range = 0.0
        for dis in range(340, 360):
            if not str(ranges[dis]) == 'inf':
                if max_range < ranges[dis]:
                    max_range = ranges[dis]

        #self.scan_dis = np.mean(ranges[350-10:350+10])
        self.scan_dis = max_range
        '''
        max_range = 0.0
        for i in range(0, 10):
            if not str(ranges[i]) == 'inf':
...
if __name__ == '__main__':
    ...
    intialTracbarVals = [33,87,25,100]
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            if (timer_past < curTime):
                timer_past = curTime + (0.1*stay_time)
                #timer cycle code
                lc_cmd= False


            if not str(laser_parser.scan_dis) == 'inf':
                #print(laser_parser.scan_dis)
                
                if not lc_cmd and laser_parser.scan_dis<0.9  and not FL:
                    print("333445555")
